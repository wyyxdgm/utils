...
function getBankCard(env, fn){
	BankCard.method.get(env.bankcardid, {}, function(err, doc){
		if(err){fn(err); return;}
		if(!doc) {fn("bankcard id error"); return;}
		env.bankcard = doc;
		fn();
	});
}
...
function writeRequest(env, fn){
	...
	addRequest({
		...
		if(err){fn(err); return;}
		if(!result) {fn("insert request/loanreq failed"); return;}
...
function checkBillRepay(env, fn){
	Bill.select({reqid: env.totradeid}, function(err, docs){
		if(err){fn(err); return;}
		var doc = docs[0];
		if(!docs){
			fn("账单该项不存在");
		...
		Trade.method.post({
			...
			BankCard.method.get(req.body.bankcardid, function(err, card){
				...
				SinaPayUtils.quickDeposit({
					...
					if(err){
						Trade.methods.put(tradeid, {status: tradeStatus.REPAY_DEPOSIT_FAILED});
						fn(err); 
						return;
					}
					if(!result.ticket){fn("no ticket"); return;}
					Trade.methods.put(tradeid, {ticket: result.ticket}, function(err){
						if(err){fn(err); return;}
						fn(null, result);
					});
				});
...
module.exports.repayMoneyVerify = function(req, fn){
	...
	async.series([function(cb){
		Trade.methods.get({ticket: req.body.ticket}, {}, function(err, doc){
			tradeid = doc._id;
		...
		ToTrade.method.get(totradeid, {}, function(err, doc){
			...
			ToTrade.method.put(totradeid, {status: totradeStatus.PROCESSING}, function(err){
				if(err) {cb(err); return;}
				ttdoc = doc;
				cb();
			});
...
//如果相等
				...
				if(err) { cb(err); return;}
				ToTrade.method.put(totradeid, {status: totradeStatus.FINISHED}, function(err, result){		
...
//			"amount": dAmount
			...
			if(err) {	
				Trade.method.put(tradeid, {status: tradeStatus.REPAY_TRANSFER_FAILED});
				console.error(err);	
				cb(err);
				return;
			}
